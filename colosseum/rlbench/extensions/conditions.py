...
class AndCondition(Condition):
    def __init__(self, conditions: List[Condition]):
        """Logical and over all conditions"""
        self._conditions = conditions
...
class DrawerCondition(Condition):
    def __init__(self, joint: Joint, target_position: float, condition_type: str = "close"):
        """
        A generic condition class for drawer states.
        :param joint: The drawer's joint to monitor.
        :param target_position: The position threshold to check against.
                               (In radians for revolute joints or meters for prismatic joints).
        :param condition_type: "close" to check if the drawer is closed, 
                               or "open" to check if the drawer is open.
        """
        self._joint = joint
        self._target_position = target_position
        self._condition_type = condition_type.lower()

        if self._condition_type not in ["close", "open"]:
            raise ValueError("condition_type must be 'close' or 'open'")

    def condition_met(self):
        current_position = self._joint.get_joint_position()

        if self._condition_type == "close":
            met = current_position < self._target_position
        elif self._condition_type == "open":
            met = current_position > self._target_position
        else:
            raise ValueError(f"Unsupported condition_type: {self._condition_type}")
